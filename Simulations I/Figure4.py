...
def AllAtOnce():
    ...
    trials = 10000
    pattime = torch.empty(12, dtype = int, device = devicce)#int(500/dta)
    pattime[:] = torch.Tensor([int(150/dta), int(300/dta), int(450/dta), int(600/dta), int(750/dta), int(900/dta), int(1050/dta), int(1200/dta), int(1350/dta), int(1500/dta), int(1650/dta), int(1800/dta)])
    ...
    for init in range(2000):
        ...
        da[:NI] = c_I * del_b_I
        da[NI:] = alpha * a[NI:] * torch.tanh(desired_S - filtered_spikes) - chi*a[NI:] + c_E * del_a_E
        a += da
    ...
    for patterns in range(em, em+1):
        omega = torch.zeros((trials, patterns, N_psn), device = devicce)
        for cycle in range(trials):
            ...
            #     V_vec[it, :] = (1 - dta/tau_m) * V_vec[it-1, :] + I_ext[0, it] * dta/tau_m
            #     if (V_vec[it-1, :] >= V_thr).any():
            #         spike_vec[it-1, torch.where(V_vec[it-1, :] >= V_thr)] = 1
            #         V_vec[it, torch.where(V_vec[it-1, :] >= V_thr)] = 0
            V_vec[:, :] = I_ext
            while len(torch.where(V_vec > V_thr)[0]) != 0:
                tn, tspike = torch.where(V_vec.T > V_thr)
                V_vec[tspike[0], tn[0]] = 1
            ...
            db[NI:] =  c_E * del_b_E + alpha * b[NI:] * torch.tanh(desired_S - filtered_spikes) - chi*b[NI:]#del_b_E#del_b_E#torch.mean(b[NI:]) * del_b_E#-chi*b[NI:] +
            da[:NI] = c_I * del_b_I
            da[NI:] = alpha * a[NI:] * torch.tanh(desired_S - filtered_spikes) - chi*a[NI:] + c_E * del_a_E 
            a += da
            a = torch.where(a > 0, a, 0)
            ...
            if np.mod(cycle, 50) == 0:
                ...
                for pat in range(patterns):
                    om[pat] = (torch.where(torch.sum(spike_vec[pattime[pat] : pattime[pat] + patlength + int(tau_m/dta)], axis = 0) != 0, 1, 0))
                omega[cycle] = om
                
        if patterns == em:
            try:
                os.mkdir(f"{path}/{directory_name}/{patterns}/")
            except:
                None
            np.save(f"{path}/{directory_name}/{patterns}/omega_{task_id}.npy", omega.cpu().numpy())
            np.save(f"{path}/{directory_name}/{patterns}/spikes_{task_id}.npy", spikes)
            np.save(f"{path}/{directory_name}/{patterns}/b_{task_id}.npy", b.cpu().numpy())
            np.save(f"{path}/{directory_name}/{patterns}/a_{task_id}.npy", a.cpu().numpy())
            np.save(f"{path}/{directory_name}/{patterns}/embedded_patterns_{task_id}.npy", embedded_pattern.cpu().numpy())
